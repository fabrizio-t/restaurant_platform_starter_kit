...
export function CartButton({ currency = 'EUR', variant = 'floating' }: CartButtonProps) {
  ...
  if (variant === 'inline') {
    return (
      <button
        onClick={toggleCart}
        className="relative flex items-center gap-2 bg-primary-400 px-4 py-2 font-bold text-black transition-colors hover:bg-primary-300 branded:bg-[var(--brand)] branded:text-[var(--brand-foreground)]"
      >
        <ShoppingCart className="w-5 h-5" />
        {itemCount > 0 && (
          <>
            <span className="font-medium">{formatCurrency(subtotal, currency, 'it-IT')}</span>
